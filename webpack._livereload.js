...
class LiveReloadPlugin {
  ...
  apply(compiler) {
    ...
    //watch for files
    // if (this.options.watchFiles != null &&
    //     Array.isArray(this.options.watchFiles)) {            
    //     const watcher = chokidar.watch(this.options.watchFiles);
    //     watcher.on('change', function(path, stats) {
    //         console.log('File ' + path + ' has been changed');
    //         if (this.server) {
    //             setTimeout(() => {
    //                 console.log('notify clients file changed: ' + path);
    //                 this.server.notifyClients([path]);
    //             }, this.options.delay);
    //         }
    //     });
    // }
    ...
    //after compile hook
    compiler.hooks.afterCompile.tap(PLUGIN_NAME, this._afterCompile.bind(this));
  }
  ...
  _afterCompile(compilation) {
    console.log('XXXXXXXXXXXXXXX afterCompile XXXXXXXXXXXXXXXX');
    //callback();
    console.log('Files: ' + JSON.stringify(compilation.files));
    if (
        this._isRunning()
    ) {
      setTimeout(() => {
        console.log('notify clients');
        this.server.notifyClients(["index.html"]);
      }, this.options.delay);
    }
  }
